...
public class UITest extends BaseSwingTestCase {
	...
	public void testMainPanel() {
		...
		GuiActionRunner.execute(new GuiTask() {
			...
			protected void executeInEDT() throws Throwable {
				...
				assertEquals(BoardDo.MAX_ROWS, ((GridLayout)mainPanel.getLayout()).getRows());
				assertEquals(BoardDo.MAX_COLS, ((GridLayout)mainPanel.getLayout()).getColumns());
			}			
		});	
	}
	...
	public void testSquares() {		
		...
		assertEquals(BoardDo.MAX_ROWS*BoardDo.MAX_COLS, components.length);
	...
	private Point getFirstCoveredSquareWhichIsAMine(SquareDo[][] squares) {
		for(int row=0; row<squares.length; row++) {
			for(int col=0; col<squares[row].length; col++) {
				if(squares[row][col].getState().equals(SquareDo.SquareState.COVERED)  && squares[row][col].isMine()) {
	...
	private Point getFirstCoveredSquareWhichIsNotAMine(SquareDo[][] squares) {
		for(int row=0; row<squares.length; row++) {
			for(int col=0; col<squares[row].length; col++) {
				if(squares[row][col].getState().equals(SquareDo.SquareState.COVERED)  && !squares[row][col].isMine()) {
					return new Point(row, col);
