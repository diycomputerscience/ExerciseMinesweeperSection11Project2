...
public class MinesweeperUtils {
	...
	public static List<Point> computeNeibhbours(Point point) {
		...
		// row-1, col+1
		if(point.row-1 >= 0 && point.col+1 < BoardDo.MAX_COLS) {
			neighbours.add(new Point(point.row-1, point.col+1));
		}
		
		// row, col+1
		if(point.col+1 < BoardDo.MAX_COLS) {
			neighbours.add(new Point(point.row, point.col+1));
		}
		
		// row+1, col+1
		if(point.row+1 < BoardDo.MAX_ROWS && point.col+1 < BoardDo.MAX_COLS) {
			neighbours.add(new Point(point.row+1, point.col+1));
		}
		
		// row+1, col
		if(point.row+1 < BoardDo.MAX_ROWS) {
			neighbours.add(new Point(point.row+1, point.col));
		}
		
		// row+1, col-1
		if(point.row+1 < BoardDo.MAX_ROWS && point.col-1 >= 0) {
			neighbours.add(new Point(point.row+1, point.col-1));
		}
